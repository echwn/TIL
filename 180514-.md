...
#### HTTP 통신
##### Cookie
- 로그인을 한 사용자가 요청을 보낼 때 마다 로그인 정보가 포함된(쿠키 유지) 요청이 감. 항상 쿠키를 쓰는 것은 아님 다른 방법(React, Angular...)을 쓰는 것이 요즘 대세
...
##### Ajax
- 전통적 웹 개발은 정보를 요청할 때마다 페이지를 새로고침함
- 화면 전체를 새로고침하지 않고 내용 갱신이 가능한 클라이언트 측 웹 개발 기법
- 요즘은 웹 브라우저에서 자바스크립트(XMLHttpRequest나 fetch() 등)를 써서 보내는 HTTP 요청을 통칭
